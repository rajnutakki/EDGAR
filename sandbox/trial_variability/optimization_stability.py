...
def optimize_with_perturbations(program, data, loss_fn_train, loss_fn_test, epsilons):
    ...
    for epsilon in epsilons:
        perturbed_params = {
            k: v * np.random.normal(loc=1, size=v.shape, scale=epsilon)
    ...
    for i, params_init in enumerate(param_sets):
        ...
        initial_loss = _eval_loss(model_fn, loss_fn_test, params_init, data[1])
        ...
        final_loss = _eval_loss(model_fn, loss_fn_test, params_opt, data[1])
        print(
            f"Losses (no complexity penalty)- Initial: {initial_loss:.4f}, Final: {final_loss:.4f}"
        )
        print(
            f"Losses (complexity penalty)- Initial: {initial_loss + spec.scoring['param_penalty_weight'] * program.n_params:.4f}, Final: {final_loss + spec.scoring['param_penalty_weight'] * program.n_params:.4f}"
        )
...
if __name__ == "__main__":
    ...
    spec = TaskSpec.from_config(
        Config.from_yaml(f"{repo_root}/projects/trial_variability/config.yaml")
    )
    seeds = [pop[0], pop[1]]
    pop = pop.get_sorted()
    top_programs = pop[:3]
