...
- `edgar_analyzer_list_runs`: Lists all runs and their live/final state under a specific or default directory.
- `edgar_analyzer_get_run_specs`: Retrieves the metadata, status, and config parameters from task_spec.yaml for a given run folder.
- `edgar_analyzer_get_top_models`: Loads the best models from the population file and displays their numpy implementations.
- `edgar_analyzer_inspect_model`: Fully inspects numpy model, JAX model, and parameter estimators for a specific program index.
- `edgar_analyzer_filter_models_by_parameters`: Filters the programs of an EDGAR run by their exact number of parameters.
- `edgar_analyzer_compare_model_syntax_trees`: Compares the Python abstract syntax trees of selected programs to find unique mathematical implementations.
