...
from tests.llm.utils import (
    ...
    generate_one_fake_param_est,
    generate_fake_models,
    make_fake_spec,
)
...
async def test_generate_same_model():
    ...
    population = [existing_program] + [
        make_empty_program() for _ in range(3)
    ]  # first program already has model, then 3 Program1s
...
async def test_translate_one_model():
    program = (
        await generate_one_fake_param_est()
    )  # The output of _generate_one_param_est
...
async def test_translate_models():
    ...
    model_jaxes = (
        [None]
        + [programs[i].model_jax + model_footer for i in range(1, 4)]
        + ["Existing jax model code"]
    )  # Expected jax translations
