...
def test_load_perfect_config():
    ...
    config = Config.from_yaml(
        "tests/io/test_configs/perfect/config.yaml",
    ...
    assert config.llms.swear_words == [
        "lstsq",
        "scipy.optimize",
        "optimize.minimize",
        "curve_fit",
    ...
    assert (
        config.prompts.parameter_estimator.code_guidelines
        == "default_param_est_code_guidelines1"
    )
    assert (
        config.prompts.parameter_estimator.docstring_guidelines
        == "default_param_est_docstring_guidelines1"
    )
    assert config.prompts.parameter_estimator.image_analysis_instructions is None
    assert (
        config.prompts.parameter_estimator.parent_program_template
        == "default_param_est_program_detail_template1"
    )
    ...
    assert (
        config.prompts.jax_translator_model.code_guidelines
        == "default_jax_code_guidelines1"
    )
    assert (
        config.prompts.jax_translator_model.docstring_guidelines
...
def test_load_invalid_subfield_config_perfect_default():
    ...
    with pytest.raises(ValidationError):
        config = Config.from_yaml(
            "tests/io/test_configs/invalid_subfield/config.yaml",
            default_path="tests/io/test_configs/perfect/default_config.yaml",
        )
...
def test_load_invalid_topology_config():
    ...
    with pytest.raises(ValidationError):
        config = Config.from_yaml(
            "tests/io/test_configs/invalid_topology/config_invalidset.yaml",
            default_path="tests/io/test_configs/perfect/default_config.yaml",
        )
    with pytest.raises(ValidationError):
        config = Config.from_yaml(
            "tests/io/test_configs/invalid_topology/config_invalidlength.yaml",
            default_path="tests/io/test_configs/perfect/default_config.yaml",
        )
...
def test_load_additional_field():
    ...
    with pytest.warns(UserWarning, match="additional_field"):
        config = Config.from_yaml(
            "tests/io/test_configs/additional_field/config.yaml",
            default_path="tests/io/test_configs/perfect/default_config.yaml",
        )
...
def test_load_additional_subfield():
    ...
    with pytest.warns(UserWarning, match="additional_subfield"):
        config = Config.from_yaml(
            "tests/io/test_configs/additional_subfield/config.yaml",
            default_path="tests/io/test_configs/perfect/default_config.yaml",
        )
