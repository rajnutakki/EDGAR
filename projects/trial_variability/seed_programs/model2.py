...
def model(data: dict, params: dict) -> np.ndarray:
    ...
    """
    V = params["V"]  # Fixed projection matrix, shape: (n_source, 5)
    W = params["W"]  # Target weights, shape: (5, n_target)
    ...
    # Project raw source cells into the stable 5-PC space
    source_proj = source @ V  # (n_trials, 5)
...
model.DEFAULT_PARAMS = lambda data: {
    "V": np.ones((data['response'].shape[-1]//2, 5)),
    "W": np.ones((5, data['response'].shape[-1] - data['response'].shape[-1]//2)),
