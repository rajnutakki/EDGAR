...
def _score_one_model(
    ...
    This function spawns a new process to execute the `_worker` function,
    allowing for robust timeout handling. If the worker process does not
    return a result within `config["timeout_s"]`, it is killed, and the
    program is assigned an infinite loss.
